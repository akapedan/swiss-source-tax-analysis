...
def create_dash_app(df_filtered):
    ...
    def update_figure(selected_cantons, selected_region, income_range, kirchensteuer, tarif_code):
        ...
        if df_filtered_view.empty:
            ...
            fig.update_layout(
                title=dict(
                    text=f'No data available for the selected combination of tarif code and church tax option',
                    x=0.5,
        ...
        church_tax_label = kirchensteuer_options.get(kirchensteuer, '')
        
        # Get the tarif code description for the title
        tarif_label = tarif_options.get(tarif_code, '').split(' - ')[1] if ' - ' in tarif_options.get(tarif_code, '') else tarif_options.get(tarif_code, '')
        
        # Update the title to include all parameters
        fig.update_layout(
            title=dict(
                text=f'Source Tax Rate Progression by Canton (2025) - {tarif_label} - {church_tax_label}',
                x=0.5,
                font=dict(size=18, color='#666666', family=font_family)
            )
        )
